...
[RTL](../../src/logic_gate.v), [TB](../../sim/tb_logic_gate_modern.sv), [XDC](../../constraints/logic_gate.xdc)를 사용했습니다. TB top은 `tb_logic_gate_modern`이며 `{a,b}`를 00→01→10→11 순서로 10ns씩 유지합니다. 각 조합에서 세 출력을 독립적인 기대값과 비교하고 오류면 `$fatal`로 중단합니다. 네 검사가 끝난 경우에만 PASS를 출력하고 40ns에 종료합니다.
...
실제 [VS Code 작업 실행 로그](../../evidence/01/vscode/simulation.txt)에서 `LAB1_PASS logic_gate cases=4`와 40ns 종료를 확인했습니다..
...
KEY1(a)=K4, KEY2(b)=N8, LED1(x)=L4, LED2(y)=M4, LED3(z)=M2, LVCMOS33입니다. Vivado에서 같은 TB를 실행한 다음 bit를 생성하고 네 입력 조합을 실제로 확인합니다. 특히 11에서 LED1·2가 켜지고 LED3가 꺼지는 장면을 사진·영상에 담을 계획입니다.
